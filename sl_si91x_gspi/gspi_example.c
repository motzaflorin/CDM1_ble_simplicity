 ...
 ***************************  Defines / Macros  ********************************
 ******************************************************************************/
#define GSPI_BUFFER_SIZE             20      // Size of buffer
...
volatile boolean_t transfer_complete = false;
volatile boolean_t begin_transmission = true;
...
void gspi_example_init(void)
{
  ...
  config.slave_select_mode = ARM_SPI_SS_SLAVE_HW; // changed to slave mode from Driver_spi.h ARM_SPI_SS_MASTER_HW_OUTPUT //ARM_SPI_SS_SLAVE_HW
  ...
  for (uint16_t i = 0; i < GSPI_BUFFER_SIZE; i++) {
    gspi_data_out[i] = (uint8_t)data[i];
  }
  do {
